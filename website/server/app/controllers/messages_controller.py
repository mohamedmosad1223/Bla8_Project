...
class MessagesController:
    ...
    def send_message(db: Session, sender: User, payload: MessageCreate):
        ...
        notification_body = ""
        notification_related_id = None

        if request:
            notification_body = f"لديك رسالة جديدة بخصوص الطلب #{request.request_id}"
    ...
    def get_chat_history(db: Session, user_id: int, request_id: int = None, other_user_id: int = None):
        ...
        if request_id:
            # Verify participation in request context
        ...
        if role in [UserRole.preacher, UserRole.muslim_caller, UserRole.interested]:
            ...
            for r in active_requests:
                ...
                previews.append({
                    ...
                    "other_party_name": f"{other_name} (طلب #{r.request_id})",
