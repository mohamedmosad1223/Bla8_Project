...
class DawahReportsController:
    ...
    def create_report(db: Session, preacher_id: int, payload: DawahReportCreate):
        ...
        active_statuses = [RequestStatus.in_progress, RequestStatus.under_persuasion]
        if request.status not in active_statuses:
            raise HTTPException(status_code=400, detail="لا يمكن إضافة تقرير على طلب مكتمل أو ملغى")
